...
middleware.isAuthenticatedUser = async (req, res, next) => {
  try {
    ...
    logger.error('[ERROR] From isAuthenticatedUser in itemMiddleware', err);
    const { code, error, status } = errorUtil.generateError(err);
    return res.status(code).json({ error, code, status });
  }
};

middleware.isItemAccess = async (req, res, next) => {
  try {
    const authError = {
      error: req.t('ERR_USER_NO_ITEM_ACCESS'),
      code: ERROR401.CODE,
    };
    const errorObj = {
      error: req.t('ERR_USER_NO_ITEM_ACCESS'),
    ...
    if (itemId) {
      const itemDetails = await Item.findOne({ _id: mongoose.Types.ObjectId(itemId) });
      if (!itemDetails) {
        throw authError;
      }
      const groupDetails = await Group.findOne({ "collectionIds.collectionId": mongoose.Types.ObjectId(itemDetails.parentId) });
      if (!groupDetails) {
        throw authError;
      } else {
    ...
    logger.error('[ERROR] From isItemAccess in itemMiddleware', err);
    const { code, error, status } = errorUtil.generateError(err);
    return res.status(code).json({ error, code, status });
  }
};
...
middleware.authenticateUser = async (req) => {
  try {
    ...
    logger.error('[ERROR] From authenticateUser in itemMiddleware', err);
    throw err;
  }
};
